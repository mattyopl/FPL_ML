...
clubEloName = {"West Bromwich Albion": "West Brom", "Manchester United": "Man United", "Manchester City": "Man City",
               "Newcastle United": "Newcastle", "Queens Park Rangers": "QPR", "Wolverhampton Wanderers": "Wolves"}

matchColumns = ["Home?", "Elo", "Opponent Elo", "Elo Diff",
                "Goals Scored", "Goals Conceded", "xG", "xGC"]
...
def matchData(season):
    ...
    for i in range(1, 21):
        ...
        page.locator(
            "//html/body/div[1]/div[3]/div[3]/div/div[2]/div/table/tbody/tr["+str(i)+"]/td[2]/a").click()

        # get teamname
        teamName = page.locator(
            "//html/body/div[1]/div[3]/ul/li[3]").inner_text()
        ...
        # if str(teamName+str(season)+".xlsx") in os.listdir("C:/Users/matth/OneDrive - Cornell University/Projects/Soccer ML/Fixture Data/2020-21"):
        #    page.close()
        #    continue
        ...
        for j in range(1, 39):
            ...
            date = page.locator(
                "//html/body/div[1]/div[3]/div[2]/div/div[2]/div[1]/div[1]/div[" + str(j) + "]/div[1]").inner_text()

            # Understat.com enemy team name pulling
            opponentTeamName = page.locator(
                "//html/body/div[1]/div[3]/div[2]/div/div[2]/div[1]/div[1]/div["+str(j)+"]/div[2]/div/div/a").inner_text()
            ...
            date = datetime.date.strftime(
                datetime.datetime.strptime(date, "%b %d, %Y"), "%Y-%m-%d")

            # navigate to the match page
            page.locator(
                "//html/body/div[1]/div[3]/div[2]/div/div[2]/div[1]/div[1]/div[" + str(j) + "]/div[2]/div/a").click()

            # clubelo elo pulling
            ELO_URL = "http://api.clubelo.com/" + date
            ELO_response = requests.get(ELO_URL)
            ELO_data = StringIO(ELO_response.text)
            ELOs = pd.read_csv(ELO_data, sep=",", nrows=250)
            ...
            # get xG scoreline
            homexg = page.locator(
                "//html/body/div[1]/div[3]/div[4]/div/div[2]/table/tbody[2]/tr/td[9]").inner_text()
            page.locator(
                "//html/body/div[1]/div[3]/div[4]/div/div[1]/div/label[2]").click()
            awayxg = page.locator(
                "//html/body/div[1]/div[3]/div[4]/div/div[2]/table/tbody[2]/tr/td[9]").inner_text()
            if home:
                xG = homexg[0:4]
            ...
            # get scoreline
            temp = page.locator(
                "//html/body/div[1]/header/div/span").inner_text()
            scoreline = ""
            for i in temp:
            ...
            tempDF = pd.DataFrame(
                [[home, elo, opponentElo, eloDiff, goalsScored, goalsConceded, xG, xGA]], columns=matchColumns)
            seasonDF = pd.concat([seasonDF, tempDF])
            teamDF = pd.concat([teamDF, tempDF])
            page.goto("https://understat.com/team/" +
                      urlTeamName + "/" + str(season))
